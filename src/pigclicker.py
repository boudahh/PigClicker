    ...
    def open_click_picker(self, file_path):
        ...
        try:
            ...
            def on_click(event):
                ...
                return offset, target_name  # Return offset and name

            canvas.bind("<Button-1>", on_click)
            click_result = picker.mainloop()  # Store the result of mainloop()

            if click_result:  # If the user clicked (mainloop returned something)
                offset, target_name = click_result
                condition_image_path, click_if_present = self._get_condition_settings(self.root)  # Get condition settings (use self.root)
            ...
            picker.destroy()  # Destroy the picker window AFTER _get_condition_settings
