   ...
   class PigClicker:
       ...
       def open_click_picker(self, file_path):
           ...
           try:
               ...
               def on_click(event):
                   ...
                   target = TargetImage(file_path, tuple(offset), self.target_name, self.condition_image_path, self.click_if_present.get())
       ...
       def _open_edit_picker(self, target: TargetImage, index_to_edit):
           ...
           try:
               ...
               change_condition_image_button = tk.Button(editor, text="Change Condition Image", command=change_condition_image)
               change_condition_image_button.pack()

               # Click if present editing
               present_check = tk.Checkbutton(editor, text="Click if Present", variable=tk.BooleanVar(value=target.click_if_present))
               present_check.pack()
